...
styles.LineString = options => {
  ...
  // const handleOptions = {
  //   multiple: geometry => ({ id: 'style:rectangle-handle', geometry: geometry.getPointN(0) }),
  ...
  //   R.tap(({ options }) => labelAnchors(geometry)(options)),
  //   R.tap(({ options }) => {
  //     const labels = (styles[`LABELS:${key}`] || []).flat()
  //       .map(R.tap(label => (label.id = 'style:default-text')))
  //     console.log('labels', labels)
  //     options.push(...labels)
  //   }),
  //   R.tap(context => {
  //     context.options = (styles[`LineString:${key}`] || styles['LineString:DEFAULT'])(context)
  //     // const handle = handleOptions[mode] || (() => [])
  //     // context.options = context.options.concat(handle(context.geometry))
  //   })
  // )

  // return pipeline({ resolution, geometry })
